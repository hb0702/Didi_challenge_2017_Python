...
def parseXML(trackletFile, findCarOnly):
        ...
        if info.tag == 'objectType':
          ...
          for pose in info:
            ...
            if pose.tag == 'count':   # this should come before the others
              ...
              newTrack.trans=np.nan * \
                  np.ones((newTrack.nFrames, 3), dtype=float)
              newTrack.rots=np.nan * \
                  np.ones((newTrack.nFrames, 3), dtype=float)
              newTrack.states=np.nan * np.ones(newTrack.nFrames, dtype='uint8')
              newTrack.occs=np.nan * \
                  np.ones((newTrack.nFrames, 2), dtype='uint8')
              newTrack.truncs=np.nan * np.ones(newTrack.nFrames, dtype='uint8')
              newTrack.amtOccs=np.nan * \
                  np.ones((newTrack.nFrames, 2), dtype=float)
              newTrack.amtBorders=np.nan * \
                  np.ones((newTrack.nFrames, 3), dtype=float)
              frameIdx=0
              ...
              for poseInfo in pose:
                ...
                if poseInfo.tag == 'tx':
                  ...
                  raise ValueError(
                      'unexpected tag in poses item: {0}!'.format(poseInfo.tag))
              frameIdx += 1
            else:
  ...
  print('loaded', len(tracklets), 'tracklets')
...
def example(kittiDir=None, drive=None):
  ...
  if kittiDir is None:
    kittiDir=expanduser(
        raw_input('please enter kitti base dir (e.g. ~/path/to/kitti): ').strip())
  if drive is None:
    drive=raw_input('please enter drive name (default {0}): '.format(
        DEFAULT_DRIVE)).strip()
    if len(drive) == 0:
      drive=DEFAULT_DRIVE
